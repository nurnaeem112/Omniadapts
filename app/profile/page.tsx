...
import { User, Mail, Shield, Zap, LogOut, CreditCard, Settings, Clock, BarChart3, CheckCircle, X } from 'lucide-react';
...
function PaymentToast({ onDismiss }: { onDismiss: () => void }) {
  useEffect(() => {
  ...
  return (
    <motion.div
      ...
      className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-4 px-6 py-4 bg-green-500 text-white rounded-full shadow-2xl shadow-green-500/30 font-black text-sm uppercase tracking-wide"
    >
...
// SessionStorage key — ensures the toast is only shown once per browser session
const PAYMENT_TOAST_KEY = 'omniadapts_payment_toast_shown';
...
function ProfileContent() {
  ...
  // ── Handle ?checkout=success param – show toast exactly once ──────────────
  useEffect(() => {
    if (searchParams.get('checkout') === 'success') {
      // Only show if we haven't shown it already this session
      if (!sessionStorage.getItem(PAYMENT_TOAST_KEY)) {
        sessionStorage.setItem(PAYMENT_TOAST_KEY, '1');
  ...
  return (
    <>
      {/* Payment success toast – shown once, auto-dismisses after 5 s */}
      <AnimatePresence>
        {showPaymentToast && (
          <PaymentToast onDismiss={() => setShowPaymentToast(false)} />
        )}
      </AnimatePresence>
      ...
      <div className="max-w-4xl mx-auto space-y-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <motion.div
            ...
            className="space-y-8"
          >
            <div className="bg-secondary p-10 rounded-[3rem] text-neutral space-y-8 shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10 space-y-6">
                ...
                <div className="space-y-2">
                  <p className="text-4xl font-black tracking-tighter uppercase">{user?.plan}</p>
                ...
                <button
                  ...
                  className="w-full py-4 bg-neutral text-secondary rounded-full font-black uppercase text-[10px] tracking-[0.2em] hover:bg-neutral/90 transition-all shadow-xl"
                >
